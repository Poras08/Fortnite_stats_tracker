...
def index():
    ...
    if request.method == 'POST':
        ...
        player_one_result = requests.get(URL.format(player_one), headers=headers).json()['lifeTime']
        player_one_stats = populate_player_data(player_one_result)

        if player_two:
            player_two_result = requests.get(URL.format(player_two), headers=headers).json()['lifeTime']
